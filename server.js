...
.bgText{position:fixed;inset:0;pointer-events:none;overflow:hidden;z-index:0}
.bgText span{
  position:absolute;color:rgba(255,255,255,.13);text-shadow:0 0 26px rgba(59,130,246,.55);
  font-size:72px;font-weight:900;letter-spacing:5px;animation:moveText 18s linear infinite;white-space:nowrap
}
...
@keyframes moveText{from{transform:translateX(0)}to{transform:translateX(190vw)}}
.lines{
  position:fixed;inset:0;pointer-events:none;z-index:0;opacity:.9;
  background:linear-gradient(90deg,transparent 0 47%,rgba(59,130,246,.18) 48%,transparent 50%),
...
.langs button.active{background:linear-gradient(135deg,#2563eb,#9333ea);box-shadow:0 0 20px rgba(147,51,234,.55)}
.card{
  position:relative;overflow:hidden;background:rgba(15,23,42,.70);border:1px solid rgba(255,255,255,.18);
  border-radius:30px;padding:24px;box-shadow:0 25px 80px rgba(0,0,0,.50);backdrop-filter:blur(9px)
}
.card:before{
  content:"VPN FAST SECURE PREMIUM VPN FAST SECURE PREMIUM";position:absolute;top:40%;left:-40%;width:180%;
  color:rgba(255,255,255,.055);font-size:46px;font-weight:900;letter-spacing:8px;white-space:nowrap;
  transform:rotate(-18deg);animation:cardText 18s linear infinite;pointer-events:none
}
@keyframes cardText{from{transform:translateX(-10%) rotate(-18deg)}to{transform:translateX(25%) rotate(-18deg)}}
.logo{
  position:relative;z-index:1;width:86px;height:86px;border-radius:28px;margin:0 auto 16px;display:grid;place-items:center;
  background:linear-gradient(135deg,#2563eb,#a855f7,#ec4899);font-size:36px;font-weight:900;
...
.userBox,.usageBox{background:rgba(0,0,0,.34);border:1px solid rgba(255,255,255,.15);border-radius:24px}
...
.usageTop,.usageBottom{display:flex;justify-content:space-between;gap:10px;font-size:14px;color:#d6def5}
.usageBottom{flex-wrap:wrap}
.usageTop b{color:white}
.bar{height:13px;background:rgba(255,255,255,.15);border-radius:999px;overflow:hidden;margin:12px 0}
.barFill{
  height:100%;background:linear-gradient(90deg,#22c55e,#3b82f6,#a855f7);border-radius:999px;
  box-shadow:0 0 20px rgba(59,130,246,.8)
}
.sectionTitle{font-size:20px;font-weight:900;margin:22px 0 12px}
.app{
  width:100%;border:0;color:white;border-radius:24px;padding:17px;margin:12px 0;display:flex;align-items:center;
  gap:15px;text-align:left;cursor:pointer;box-shadow:0 14px 34px rgba(0,0,0,.32)
}
.icon{
  width:58px;height:58px;border-radius:18px;display:grid;place-items:center;background:rgba(255,255,255,.20);
  font-size:26px;font-weight:900
}
.txt{flex:1}.title{font-size:20px;font-weight:900}.sub{font-size:14px;color:rgba(255,255,255,.84);margin-top:5px}.arrow{font-size:34px}
...
.qrBtn{
  width:100%;border:1px solid rgba(255,255,255,.16);background:rgba(0,0,0,.34);color:#fff;border-radius:22px;
  padding:16px;font-size:17px;font-weight:900;margin-top:18px
}
.qrBox{display:none;margin-top:18px;text-align:center;background:white;padding:16px;border-radius:22px}
.qrBox img{width:230px;height:230px}.qrText{color:#0f172a;font-size:12px;word-break:break-all;margin-top:10px}
.toast{
  display:none;position:fixed;left:50%;bottom:24px;transform:translateX(-50%);background:#22c55e;color:#fff;
  padding:13px 18px;border-radius:999px;font-weight:900;z-index:99
}
</style>
</head>
<body>
...
<div class="wrap">
  ...
  <div class="card">
    ...
    <div class="usageBox">
      <div class="usageTop"><span id="usedLabel">Ulanylan</span><b>${usage.usedText} / ${usage.totalText}</b></div>
